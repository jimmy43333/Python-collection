...
class Strategy_05(Strategy):
	...
	def judge_entry_point(self):
		...
		if self.today_open < self.yesterday_close:
			if self.current["Low"] < self.yesterday_close + 10 < self.current["High"]:
				self.in_price = self.yesterday_close + 10
...
if __name__ == '__main__':
	long_short = "L"
	obj = Strategy_05(f"Strategy05_{long_short}")
